...
const MAZADX_WEBHOOK_URL = 'https://mazadx.com/WhatsappWebhook.ashx'; 
const STORAGE_URL = 'https://dbbqpjglpqthxvkxhyrh.supabase.co/storage/v1/object/authenticated/backups/auth_info.zip';
...
// دالة لسحب ملف الجلسة وفك ضغطه عند بداية التشغيل على Render
async function downloadSession() {
    try {
        ...
        // فك الضغط تلقائياً إلى مجلد auth_info
        ...
        console.log('تنبيه: لم يتم العثور على جلسة سابقة (أو أول تشغيل)، سيتم توليد باركود جديد.');
    }
}

// دالة لضغط مجلد الجلسة ورفعه لتحديث المخزن الخارجي أوتوماتيكياً
async function uploadSession() {
    try {
        console.log('جاري حفظ وتحديث الجلسة في مخزن Supabase حمايةً لها من الحذف...');
        const AdmZip = require('adm-zip');
        ...
        if (fs.existsSync(folderPath)) {
            ...
            // رفع الملف وعمل تداخل (Upsert) لتحديث الملف القديم بنفس الاسم
            ...
            fs.unlinkSync('auth_info.zip');
            console.log('تم تحديث المخزن الخارجي بنجاح تام "فور إيفر".');
        }
    } catch (err) {
...
async function startWhatsApp() {
    // تحميل الجلسة أولاً قبل محاولة الاتصال بالواتساب
    await downloadSession();

    const { state, saveCreds } = await useMultiFileAuthState(path.join(__dirname, 'auth_info'));
    const sock = makeWASocket({
        auth: state,
        printQRInTerminal: false // سنقوم بطباعته يدوياً منسقاً بالأسفل
    });

    // إدارة أحداث الاتصال والباركود
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            await uploadSession(); // رفع النسخة المتصلة فوراً للمخزن السحابي
        }
    });

    // الاستماع لتحديث ملفات الأمان والـ Tokens وحفظها خارجياً
    sock.ev.on('creds.update', async () => {
        await saveCreds();
        await uploadSession(); 
    });

    // الاستماع للرسائل الواردة وإعادة توجيهها للـ Web Forms (الـ Generic Handler)
    sock.ev.on('messages.upsert', async m => {
        const msg = m.messages[0];
        
        // نتحقق أن الرسالة ليست مرسلة منا وأنها إشعار عادي
        if (!msg.key.fromMe && m.type === 'notify') {
            const isGroup = msg.key.remoteJid.endsWith('@g.us');
            const messageText = msg.message?.conversation || msg.message?.extendedTextMessage?.text;

            // تصفية الجروبات والتركيز على رسائل العملاء والإنتاج فقط
            if (!isGroup && messageText) {
                ...
                try {
                    // إرسال البيانات فوراً لملف الـ ashx على استضافتك المشتركة
