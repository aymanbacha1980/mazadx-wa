...
// 1. الإعدادات الرسمية والنهائية لمنصة مزاد إكس الحية والـ Supabase
const MAZADX_WEBHOOK_URL = 'https://mazadx.com/ReceiveWhatsappWebhook.aspx'; 
...
async function startWhatsApp() {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (qr) { 
            // نترك طباعة الباركود فقط احتياطاً لو سقطت الجلسة في أي وقت لتقرأها من ترمنال Railway
    ...
    // حدث الاستقبال لايف - تنظيف وقص وتمرير فوري للإنتاج 🚀
    sock.ev.on("messages.upsert", async ({ messages, type }) => {
        ...
        if (msg && !msg.key.fromMe && type === 'notify') {
            ...
            if (!isGroup && messageText) {
                ...
                // سحب رقم الهاتف الحقيقي الصريح فوراً من الحقل البديل
                ...
                // تكوين الـ الـ JSON المقصوص والجاهز للاستقبال في الـ Web Forms بالملي
                const payload = {
                ...
                try {
                    ...
                    // console.log(`[تم التمرير لايف] للرسالة: ${payload.messageId}`);
                } catch (error) {
                    // console.error('خطأ برودكشن أثناء التمرير:', error.message);
                }
            }
        }
    });
